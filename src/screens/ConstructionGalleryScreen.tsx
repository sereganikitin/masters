...
// gallery isn't empty on first run. Merged with DB entries by year+month, so it
// stays visible while admins add other months, and is overridden only if an
// admin creates their own Июнь 2026 entry. Earlier months are added via admin.
const CONSTRUCTION_FALLBACK: ConstructionEntry[] = [
  {
    id: -202606,
    year: 2026,
    month: 6,
...
/** DB entries win; fallback fills months the admin hasn't created yet. */
function withFallback(items: ConstructionEntry[]): ConstructionEntry[] {
  const present = new Set(items.map((i) => `${i.year}-${i.month}`));
  const merged = [
    ...items,
    ...CONSTRUCTION_FALLBACK.filter((f) => !present.has(`${f.year}-${f.month}`)),
  ];
  return merged.sort((a, b) => b.year - a.year || b.month - a.month || a.sortOrder - b.sortOrder);
}
