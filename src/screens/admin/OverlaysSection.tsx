...
// Polygon editor — pre-existing tool moved into the admin shell as one section.
...
export function OverlaysSection() {
  ...
  const startDraft = () => {
    ...
    setDraftMeta((m) => ({
      ...m,
      entityType:
        scope === "genplan" ? "section" : scope === "section" ? "floor" : "apartment",
      entityId: "",
      label: "",
    }));
  };
  ...
  const moveVertex = async (overlayId: number, vIdx: number, p: [number, number]) => {
    ...
    if (!overlay) return;
    const nextPoints = overlay.points.map((pt, i) => (i === vIdx ? p : pt)) as [
      number,
      number,
    ][];
    setOverlays((prev) =>
      prev.map((o) => (o.id === overlayId ? { ...o, points: nextPoints } : o)),
    );
    try {
  ...
  return (
    <div className="p-5">
      <header className="mb-4">
        <h2 className="font-display text-h4 font-semibold">Редактор обводок</h2>
        <p className="font-sans text-small text-base-600">
          Полигональные подсветки для секций, этажей и квартир.
        </p>
      </header>
...
// ─────────────────────────────────────────────────────────────────────────────
// Pickers (kept inline — only used by this section)
// ─────────────────────────────────────────────────────────────────────────────
